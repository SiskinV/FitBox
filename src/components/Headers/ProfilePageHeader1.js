...
function ProfilePageHeader1() {
  ...
  if(lg!=null){
    ...
    for(var i=0;i<matrix.length;i++)
      {if(matrix[i][0]==lg){ime=matrix[i][1]+' '+matrix[i][2]; god=matrix[i][3];rang=matrix[i][4];avg=matrix[i][5];aktivnost=matrix[i][6];}}
  }
  console.log(god);
      ...
      >
        ...
        <Container>
          <div className="photo-container">
            <img alt="..." src={"https://getstream.io/random_svg/?id=m-3&name=" +ime}></img>
          </div>
        <h3 className="title"> {ime} </h3>
